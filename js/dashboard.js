// ═══════════════════════════════════════════════
// LÓGICA DO DASHBOARD — não precisa editar aqui
// ═══════════════════════════════════════════════
...
function tick() {
  ...
  if (next > 0) {
    ...
    document.getElementById('f-refresh').textContent = '↻ '+(m>0?m+'m ':'')+s+'s';
  }
}
...
function animProg() {
  document.getElementById('prog').style.width =
    Math.min(((Date.now()-startTime)/ROTATE_MS)*100,100)+'%';
...
function goSlide(idx) {
  ...
  document.getElementById('f-counter').textContent =
    String(idx+1).padStart(2,'0')+' / '+String(CONFIG.SLIDES.length).padStart(2,'0');
